...
.slyder-g
  ...
  .slyder-g__btn.pe-3(@click="clickAnterior")
    i.fas.fa-chevron-left
  .slyder-g__main(:id="'slyder-g-' + mainId")
    ScrollHorizontal(
  ...
  .slyder-g__btn.ps-3(@click="clickSiguiente")
...
import slyderMixins from '../components/slyderMixins'
export default {
  name: 'SlyderF',
  ...
  methods: {
    ...
    clickSiguiente() {
      const ids = this.elements.map(element => element.id)
      const container = document.querySelector('#slyder-g-' + this.mainId)
